...
Write-Host "  SEGMENTA - One-Click Installer and Build Suite      " -ForegroundColor White
Write-Host "======================================================" -ForegroundColor Cyan
...
if (-not (Test-Path $HostBin)) {
    Write-Host "[ERROR] Failed to compile segmenta-host.exe. Please ensure Rust is installed." -ForegroundColor Red
    exit 1
}
Write-Host "[OK] Host binary compiled at: $HostBin" -ForegroundColor Green
...
try {
    ...
    Write-Host "[OK] Registry keys registered for Chrome and Edge." -ForegroundColor Green
} catch {
    Write-Host "[WARN] Registry registration notice: $_" -ForegroundColor DarkYellow
}
...
Write-Host "[OK] Extension built in: $RepoRoot\apps\extension\dist" -ForegroundColor Green
...
if (Test-Path $DesktopBin) {
    Write-Host "[OK] Desktop release binary compiled at: $DesktopBin" -ForegroundColor Green
} else {
    Write-Host "[INFO] Standalone installer can be built with: npm --prefix apps/desktop run tauri build" -ForegroundColor Gray
}
...
Write-Host "  3. Click Load unpacked and choose folder:" -ForegroundColor Gray
Write-Host "     $RepoRoot\apps\extension\dist" -ForegroundColor Cyan
